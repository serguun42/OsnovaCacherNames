const { NAMES_CACHER_API_URL, RESOURCES_ROOT, SITE, VERSION } = require('./config/sites.js');
const { AddStyle, WaitForElement, GEBI, QSA, QS, SetCustomInterval, GR } = require('./util/dom.js');
...
const CreateBigTooltip = (id, skipName = '', isTinyOnMobile = false) => {
  ...
  toggleButton.innerHTML = `<div class="v-button__icon">\
  ...
  toggleButton.addEventListener('click', (e) => {
    ...
    if (namesList.classList.contains('site-names__list--compressed')) {
      namesList.style.top = `${
        e.target ? (e.target.getBoundingClientRect ? e.target.getBoundingClientRect().bottom : e.clientY) : e.clientY
      }px`;
    }
...
const SeeUnseenUsers = () => {
  ...
  const profileId =
    window.location.pathname.match(/\/[us]\/(\d+)/) && !window.location.pathname.match(/\/[us]\/(\d+)(-[^/]+)?\/(\d+)/)
      ? parseInt(window.location.pathname.match(/\/[us]\/(\d+)?/)[1])
      : 0;
  ...
  new Promise((resolve) => {
    if (cacherNamesFetchTimeout || !usersIdsSet.size) {
      resolve({});
      return;
    }
    ...
    fetch(NAMES_CACHER_API_URL, {
      ...
      .then((usersFromAPI) => {
        if (!usersFromAPI || typeof usersFromAPI !== 'object') return Promise.reject(new Error(`No <usersFromAPI>`));
    ...
    /** @param {{[userID: string]: string[]}} usersFromAPI */ (usersFromAPI) => {
      Object.keys(usersFromAPI).forEach((userID) => {
        CACHER_NAMES_USERS_NICKS[userID] = usersFromAPI[userID];
      });
